...
fnValidateWithgCSI <- function(biomarker, method=c("common","all"), my.xlim, my.ylim, color, drug)
{
  gCSI.pvalue <- 2 
  gCSI.estimate <- gCSI.R2 <- 0
  gCSI.pvalue.ftest <- 1
  ...
  gCSI.sensitivity <- subset(gCSI.drug.sensitivity, !is.na(gCSI.drug.sensitivity[, drug]) , select=drug)
  intersected.celllines.gCSI <- intersect(rownames(gCSI.sensitivity), rownames(gCSI.genes.fpkm))
  if(method == "common"){
    intersected.celllines.gCSI <-  intersect(intersected.celllines.gCSI, rownames(ccle.genes.fpkm))
  }
  if(exp.type == "gene.exp")
  {
    sensitivity <- data.frame(list(gCSI.sensitivity[intersected.celllines.gCSI, drug], gCSI.genes.fpkm[intersected.celllines.gCSI, biomarker$gene.id], tissueTypes[intersected.celllines.gCSI,]))
  }else{
    sensitivity <- data.frame(list(gCSI.sensitivity[intersected.celllines.gCSI, drug], gCSI.isoforms.fpkm[intersected.celllines.gCSI, biomarker$isoform.id],tissueTypes[intersected.celllines.gCSI,]))
  }
  sensitivity <- sensitivity[complete.cases(sensitivity),]
  colnames(sensitivity) <- c(phenotype, exp.type, "tissue.type")
  gCSI.n <- nrow(sensitivity)
  if(gCSI.n==0){
    ...
    textxy(.2, .2, sprintf("No complete cases"), cex=0.6)
    title(sprintf("gCSI-%s\n%s and %s", toupper(method), biomarker$type, gsub("drugid_", "", drug)), cex.main=.8)

    gCSI.pvalue <- gCSI.pvalue / 2
    ...
    if(!is.finite(my.xlim)||!is.finite(my.ylim)){browser()}
    ...
    {
      gCSI.model <- lm(sensitivity[, phenotype]  ~ sensitivity[ , "gene.exp"] + sensitivity[,"tissue.type"])
    }else{
      gCSI.model <- lm(sensitivity[, phenotype] ~ as.numeric(sensitivity[ , "isoform.exp"]) + as.factor(sensitivity[,"tissue.type"]))    
    }

    if(all(!is.na(gCSI.model)) & !is.na(gCSI.model$coefficients[2]))
    {
      gCSI.pvalue <- summary(gCSI.model)$coefficients[2,4]
      gCSI.estimate <- summary(gCSI.model)$coefficients[2,1]
      gCSI.R2 <- summary(gCSI.model)$adj.r.squared
      gCSI.pvalue.ftest <- anova(gCSI.model)$Pr[1]
    }
    #lm compute two sided pvalue
    gCSI.pvalue <- gCSI.pvalue / 2
    textxy(my.xlim[1], my.ylim[2] - .08, sprintf("In gCSI model (Linear Regression)\n pvalue=%s\nR2=%s\nestimate=%s", round(gCSI.pvalue, digits=10), round(gCSI.R2, digits=2), round(gCSI.estimate, digits=2)), cex=0.6)
  }
  
  return(list(pvalue=gCSI.pvalue, estimate=gCSI.estimate, n=gCSI.n, gCSI.pvalue.ftest=gCSI.pvalue.ftest))
}
fnPlotCCLEGDSC <- function(biomarker, tissue.type, my.xlim, my.ylim, drug)
{
  ...
  cnames <- paste0("ccle.", phenotype)
  if(biomarker$gene.id != 0){
    cnames <- c(cnames, "gene.exp")
  }
  if(biomarker$isoform.id != 0){
    cnames <- c(cnames, "isoform.exp")
  }
  cnames <- c(cnames, "tissue.type")
  colnames(unique) <- cnames
  ...
  cnames <- c(paste0("ccle.", phenotype), paste0("gdsc.", phenotype))
  if(biomarker$gene.id != 0){
    cnames <- c(cnames, "gene.exp")
  }
  if(biomarker$isoform.id != 0){
...
{
  switch(first, "gCSI"={first.db <- gCSI.drug.sensitivity}, "ccle"={first.db <- ccle.drug.sensitivity}, "gdsc"={first.db <- gdsc.drug.sensitivity})
  switch(second, "gCSI"={second.db <- gCSI.drug.sensitivity}, "ccle"={second.db <- ccle.drug.sensitivity}, "gdsc"={second.db <- gdsc.drug.sensitivity})
...
fnPlotEXP <- function(first, second, biomarker)
{
  ...
  switch(second, "gCSI.gene"={second.db <- gCSI.genes.fpkm}, "gCSI.isoform"={second.db <- gCSI.isoforms.fpkm})
  
  celllines <- intersect(rownames(first.db), rownames(second.db))
  expression <- data.frame(cbind(first.db[celllines, biomarker$id], second.db[celllines, biomarker$id]))
  colnames(expression) <- c(paste0(biomarker$label, " in CCLE"), paste0(biomarker$label, " in gCSI"))
  rownames(expression) <- celllines
  expression <- expression[complete.cases(expression),]
  exp.n <- nrow(expression)
  if(exp.n == 0){
...
{
  biomarker <- list()
  if(gene.id == "") biomarker[["gene.id"]] <- 0 else biomarker[["gene.id"]] <- gene.id
  if(isoform.id == "") biomarker[["isoform.id"]] <- 0 else biomarker[["isoform.id"]] <- isoform.id
...
fnValidation <- function(top.significant.biomarkers, validation.cut.off, validation.stat="cindex") #tissue <- c("all", "tissue types")
{
  rr <- list()
  ...
  for(drug in drugs)
  {
    ...
    if(!all(is.na(top.significant.biomarkers[[drug.name]])))
    {
      ...
      if(training.type == "CCLE_GDSC")
      {
        ...
        fnPlotAUC(first="ccle", second="gCSI", drug, tissue.type="", color="#CC3399")
        fnPlotAUC(first="gCSI", second="gdsc", drug, tissue.type="", color="#663399")
      ...
      for(i in 1:plots.no)
      {
        biomarker <- fnDefineBiomarker (gene.id=as.character(top.significant.biomarkers.drug[i, "gene.id"]), 
                                        ...
                                        R2=as.numeric(top.significant.biomarkers.drug[i, stat]), 
                                        type=as.character(top.significant.biomarkers.drug[i, "type"]))
        if(biomarker$type == "isoform") {f <- biomarker$id %in% colnames(gCSI.isoforms.fpkm) & biomarker$id %in% colnames(gCSI.isoforms.fpkm)}
        if(biomarker$type == "gene") {f <- biomarker$id %in% colnames(gCSI.genes.fpkm) & biomarker$id %in% colnames(gCSI.genes.fpkm)}
        if (f)
        {
          ...
          rr[[drug.name]][i, "all.gCSI.n"] <- gCSI$n
          rr[[drug.name]][i, "all.gCSI.pvalue"] <- gCSI$pvalue
          rr[[drug.name]][i, "all.gCSI.estimate"] <- gCSI$estimate
          rr[[drug.name]][i, "all.gCSI.pvalue.ftest"] <- gCSI$gCSI.pvalue.ftest
          
          gCSI <- fnValidateWithgCSI(biomarker, method="common", color="#336600", drug=drug)
          rr[[drug.name]][i, "common.gCSI.n"] <- gCSI$n
          rr[[drug.name]][i, "common.gCSI.pvalue"] <- gCSI$pvalue
          rr[[drug.name]][i, "common.gCSI.estimate"] <- gCSI$estimate
          rr[[drug.name]][i, "common.gCSI.pvalue.ftest"] <- gCSI$gCSI.pvalue.ftest
...
fnGtex <- function(validation.method=c("R2", "pvalue")) 
{
  ...
  gCSI.breast.genes <- gCSI.genes.fpkm #2^gCSI.genes.fpkm -1
  gCSI.breast.isoforms <- gCSI.isoforms.fpkm #2^gCSI.isoforms.fpkm -1
  
  gtex.breast.genes <- t(exprs(GTex.BR$genes))
  gtex.breast.isoforms <-  t(exprs(GTex.BR$isoforms))
  
  breast.no <- max(max(nrow(gtex.breast.genes), nrow(ccle.breast.genes)), nrow(gCSI.breast.genes))
  mycol <- RColorBrewer::brewer.pal(n=8, name="Set2")[c(3,4,6)]
  
  
  tt <- gdata::sheetNames(xls=file.path(path.diagrams, "top.biomarkers.gCSI.xlsx"))
  all.biomarkers <- vector("list" , length=length(tt))
  names(all.biomarkers) <- tt
  
  for(i in 1:length(all.biomarkers))
  {
    all.biomarkers[[i]] <- gdata::read.xls(xls=file.path(path.diagrams, "top.biomarkers.gCSI.xlsx"), sheet=i, stringsAsFactors=F)
    ...
    if(no > 0)
    {
      ...
      for( j in 1:nrow(all.biomarkers[[i]]))
      {
        ...
        if(!is.na(all.biomarkers[[i]][j, "all.gCSI.pvalue"]))
        {
          if(as.numeric(all.biomarkers[[i]][j, "all.gCSI.pvalue.ftest"]) < .05 & 
             sign(as.numeric(all.biomarkers[[i]][j, "estimate"])) == sign(as.numeric(all.biomarkers[[i]][j, "all.gCSI.estimate"]))) {validation.stat <- "validated"}
          ...
          if(all.biomarkers[[i]][j, "type"] == "gene")
          {
            ...
            all.biomarkers[[i]][j, "gCSI"] <- median(gCSI.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])])
            all.biomarkers[[i]][j, "gtex"] <- median(gtex.breast.genes[, ensembl.id])
            tt <- matrix(NA, ncol=3, nrow=breast.no)
            colnames(tt) <- c("CCLE", "gCSI", "GTEX")
            tt[1:nrow(ccle.breast.genes),1] <- ccle.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])]
            tt[1:nrow(gCSI.breast.genes),2] <- gCSI.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])]
            ...
            all.biomarkers[[i]][j, "gCSI"] <- median(gCSI.breast.isoforms[, as.character(all.biomarkers[[i]][j, "transcript.id"])])
            ...
            tt[1:nrow(gCSI.breast.isoforms),2] <- gCSI.breast.isoforms[, as.character(all.biomarkers[[i]][j, "transcript.id"])]
          ...
          legend("topright", legend=c(sprintf("CCLE > GTEX : %s", round(train, digits=25)), sprintf("gCSI > GTEX : %s", round(test, digits=25))), fill=mycol[1:2], bty="n")
...
fnFetchBiomarkers <- function(top.significant.biomarkers, drug, indices) 
{
  ...
  for(i in indices)
  {
    ...
    biomarker <- fnDefineBiomarker2 (gene.id=as.character(top.significant.biomarkers[i, "gene.id"]), 
                                    ...
                                    gCSI.estimate=as.numeric(top.significant.biomarkers[i, "all.gCSI.estimate"]),
                                    gCSI.pvalue=as.numeric(top.significant.biomarkers[i, "all.gCSI.pvalue"]), 
                                    R2=as.numeric(top.significant.biomarkers[i, stat]), 
                                    ...
                                    symbol=as.character(top.significant.biomarkers[i, "symbol"]))
...
fnPlotAUCoverCellLinesgCSI <- function(drug, tissue.type, biomarkers, p.values=NULL, suffix) 
{
  sensitivity <- subset(gCSI.drug.sensitivity, !is.na(gCSI.drug.sensitivity[,drug]), select=drug)
  sensitivity <- data.frame(sensitivity , "col"="#000000", check.names=FALSE)
  # colnames(sensitivity) <- c(drug, "col")
  sensitivity <- sensitivity[order(as.numeric(sensitivity[,drug])),]
  sensitivity <- subset(sensitivity, rownames(sensitivity) %in% rownames(gCSI.genes.fpkm))
  exp.db <- NULL;
  exp.col <- NULL;
  gene_biotype <- NULL;
  for (i in 1:length(biomarkers))
  {
    if (biomarkers[[i]]$type == "isoform")
    {
      exp.db <- cbind(exp.db, gCSI.isoforms.fpkm[rownames(sensitivity),biomarkers[[i]]$isoform.id])  
      #      exp.db <- cbind(exp.db, (gCSI.isoforms.fpkm[rownames(sensitivity),biomarkers[[i]]$isoform.id] - 
      #                                apply(gCSI.isoforms.fpkm[rownames(sensitivity),], 1, mean, na.rm=T))/
      #                                apply(gCSI.isoforms.fpkm[rownames(sensitivity),], 1, sd, na.rm=T))
    }else{
      exp.db <- cbind(exp.db, gCSI.genes.fpkm[rownames(sensitivity),biomarkers[[i]]$gene.id])  
      #      exp.db <- cbind(exp.db, (gCSI.genes.fpkm[rownames(sensitivity),biomarkers[[i]]$gene.id] - 
      #                                apply(gCSI.genes.fpkm[rownames(sensitivity),], 1, mean, na.rm=T))/
      #                                apply(gCSI.genes.fpkm[rownames(sensitivity),], 1, sd, na.rm=T))
    }
  }
  ...
  my.xlim <- c(1,nrow(sensitivity))
  my.ylim <- range(as.numeric(sensitivity[,1]))
  ...
  if(ncol(exp.db) == 1){
    pdf(file=file.path(path.diagrams, sprintf("%s_%s_gCSI_%s.pdf", gsub("drugid_","",drug), phenotype, suffix)), height=2, width=22)  
    par(mar=c(1, 1, 1, 17))
    par(oma=c(2,2,2,2))
    image(exp.db, col=exp.col[,"col"], axes=FALSE)
    grid(nx=nrow(exp.db), ny=ncol(exp.db), lty=1)
    ...
    pdf(file=file.path(path.diagrams, sprintf("%s_%s_gCSI_%s.pdf", gsub("drugid_","",drug), phenotype, suffix)), height=9 , width=17)
    #par(mar=c(2, 2, 2, 8))
    par(oma=c(0,0,0,10))
  ...
  dev.off()
  pdf(file=file.path(path.diagrams, sprintf("%s_%s_AUC_gCSI_2_%s.pdf", gsub("drugid_","",drug), phenotype, suffix)), height=5, width=15)  
  par(mar=c(12,5,2,8))
  plot(NA, xlim=my.xlim, ylim=my.ylim,ylab='',xlab='', axes=FALSE)
  axis(1,at=1:nrow(sensitivity), labels=rownames(sensitivity), las=2, cex.axis=1.5, tck=-.05)
  axis(2,at=c(0,.1,.2,.3,.4,.5), labels=c(0,.1,.2,.3,.4,.5), cex.axis=1.5, tck=-.02)
...
fnDefineBiomarker2 <- function(gene.id, isoform.id, estimate, pvalue, gCSI.estimate, gCSI.pvalue, R2, type, biotype, gtex, delta.rank, isoform.no, symbol) 
{
  biomarker <- list()
  if(gene.id == "") biomarker[["gene.id"]] <- 0 else biomarker[["gene.id"]] <- gene.id
  if(isoform.id == "") biomarker[["isoform.id"]] <- 0 else biomarker[["isoform.id"]] <- isoform.id
  ...
  biomarker[["estimate"]] <- estimate
  biomarker[["pvalue"]] <- pvalue
  biomarker[["gCSI.estimate"]] <- gCSI.estimate
  biomarker[["gCSI.pvalue"]] <- gCSI.pvalue
  biomarker[["type"]] <- type
...
fnPanCancer <- function(validation.method=c("R2", "pvalue")) 
{
  # ccle.breast.cells <- rownames(ccle.tissuetype)#[which(ccle.tissuetype[ , "tissue.type"] == "breast")]
  # ccle.breast.cells <- intersectList(ccle.breast.cells, rownames(ccle.genes.fpkm), rownames(ccle.isoforms.fpkm))
  ccle.breast.genes <- ccle.genes.fpkm#[ccle.breast.cells,]
  ...
  breast.no <- max(nrow(ccle.breast.genes), nrow(gCSI.breast.genes))
  mycol <- RColorBrewer::brewer.pal(n=8, name="Set2")[c(3,4,6)]
  ...
  for(i in 1:length(all.biomarkers))
  {
    ...
    # if(validation.method == stat) {
      # all.biomarkers[[i]] <- subset(all.biomarkers[[i]], all.biomarkers[[i]][ ,stat] <= all.biomarkers[[i]]$breast)
    # } else {
      # all.biomarkers[[i]] <- subset(all.biomarkers[[i]], all.biomarkers[[i]][ ,paste0(tissue, "_pvalue")] <= 0.05)
    # }
    ...
    if(no > 0)
    {
      ...
      {
        validation.stat <- "unvalidated"
        fstat.validation.stat <- "unvalidated"
        if(!is.na(all.biomarkers[[i]][j, "all.gCSI.pvalue"]))
        {
          if(as.numeric(all.biomarkers[[i]][j, "all.gCSI.pvalue"]) < .05 & 
             sign(as.numeric(all.biomarkers[[i]][j, "estimate"])) == sign(as.numeric(all.biomarkers[[i]][j, "all.gCSI.estimate"]))) {validation.stat <- "validated"}
          
          if(as.numeric(all.biomarkers[[i]][j, "all.gCSI.pvalue.ftest"]) < .05 & 
             sign(as.numeric(all.biomarkers[[i]][j, "estimate"])) == sign(as.numeric(all.biomarkers[[i]][j, "all.gCSI.estimate"]))) {fstat.validation.stat <- "validated"}
          ...
          if(all.biomarkers[[i]][j, "type"] == "gene")
          {
            ...
            all.biomarkers[[i]][j, "gCSI"] <- median(gCSI.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])])
            # all.biomarkers[[i]][j, "gtex"] <- median(gtex.breast.genes[, ensembl.id])
            tt <- matrix(NA, ncol=2, nrow=breast.no)
            colnames(tt) <- c("CCLE", "gCSI")#, "GTEX")
            tt[1:nrow(ccle.breast.genes),1] <- ccle.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])]
            tt[1:nrow(gCSI.breast.genes),2] <- gCSI.breast.genes[, as.character(all.biomarkers[[i]][j, "gene.id"])]
            # tt[1:nrow(gtex.breast.genes),3] <- gtex.breast.genes[, ensembl.id]
          ...
          # test <- wilcox.test(tt[,2], tt[,3], alternative="greater")$p.value
          # legend("topright", legend=c(sprintf("CCLE > GTEX : %s", round(train, digits=25)), sprintf("gCSI > GTEX : %s", round(test, digits=25))), fill=mycol[1:2], bty="n")
          # legend("topright", legend=c(sprintf("CCLE > GTEX : %s", round(train, digits=25))), fill=mycol[1:2], bty="n")
          all.biomarkers[[i]][j, "validation.stat"] <- validation.stat
          all.biomarkers[[i]][j, "fstat.validation.stat"] <- fstat.validation.stat
          # all.biomarkers[[i]][j, "Gtex.train.sit"] <-  train
          # all.biomarkers[[i]][j, "Gtex.test.sit"] <- test
        }
      }
