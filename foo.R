...
rsq <- function(model, indices) 
{
  ...
  if(is.null(tissue))
  {
    CIndex <- survcomp::concordance.index(x=y.hat, surv.time=data[rownames(newPRED),RESP.pos], surv.event=rep(1, length(y.hat)), na.rm=T)[[1]]
  }else
  {
    CIndex <- 0
  }
  ...
  return(list( rmsd = RMSD, cindex = abs(CIndex-0.5), r.squared = R.square, adj.r.squared = Adjusted.R.square))
}
...
fnRunbootstrap <- function (models)
{
  ...
  for(model in names(models)) {
    boot.models[[model]] = list(data = models[[model]]$dataset, formula=models[[model]]$formula, object=models[[model]]$model)
  }
  M_res = fnboot(models=boot.models, R=100)
  return(M_res)
}
fnCreateNullModel <- function(drug, assay = c("ccle","gdsc", "gray"))
{
  tissues.no <- 1
  if(is.null(tissue))
  {
  ...
  if(is.null(tissue))
  {
    Genedf0[,"Tissue"] <- factor(ccle.tissuetype[rownames(Genedf0),"tissue.type"], ordered =FALSE)
    ...
    CellLines <- rownames(subset(ccle.tissuetype, ccle.tissuetype$tissue.type == tissue))
...
fnCreateGeneModel <- function(drug, nullModel, data)
{
  ...
  xx <- rownames(nullModel$dataset)[which(!is.na(data[rownames(nullModel$dataset)]))]
  GenedfM <- nullModel$dataset[xx, ,drop=FALSE]
  GenedfM[,"Gene"] <- data[xx]
  switch(model.method, "glm"={
    if(glm.family == "linear")
    {
...
mybarplot <- function(Filename, data, barsNo, groupNo, group.labels, ylab.label, legend.lables, main.label, yaxis = c("Regular", "Log"))
{
  #compare all
  ...
  par(oma=c(2,2,2,2))
  temp <- rbind(data[seq(2, 30, by=2)], data[seq(1, 30, by=2)])
  if(yaxis == "Log") {
    if(0 %in% temp){
      temp <- temp + 1
    }
    ...
    legend("topright", inset=c(-.05,0), legend=c(legend.lables[2],legend.lables[1]), fill=barplot.colors[1:barsNo], bty="n")
  }else {
...
mystacked.barplot <- function(Filename, data, barsNo, stackedNo, groupNo, group.labels, ylab.label, legend.lables, main.label, yaxis = c("Regular", "Log"))
{ 
  colors.brewer= c("Blues", "Reds","Greens")
  colors.legend= c("deepskyblue4", "firebrick3","seagreen4")
  ...
  legend("topleft", legend = legend.lables, fill = colors.legend,bty="n", cex = 1.2)
...
mystacked.barplot.simple <- function(Filename, data, main.label)
{
  mycol <- RColorBrewer::brewer.pal(n=4, name="Set1")
  
  C = c(mycol[1],mycol[4],mycol[2])
  #C = c("purple","blue","red")
 ...
 # mp <- barplot(data, col = C, ylim = yr, ylab = "Percentage", main = main.label,border=NA,axisnames = FALSE, space = 0.5)
  ...
  text(mp, par("usr")[3], labels = colnames(data), srt = 40, adj = c(1.1,1.1), xpd = TRUE, cex=1.3)
  #axis(1, at=seq(1,length(xx), by=1),labels=FALSE)
  #axis(2)
...
{
  mean.data = double()
  for(i in 1:ncol(data)){ mean.data[i] = mean(data[complete.cases(data),i])}
