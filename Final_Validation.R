...
for(drug in drugs) {
  ...
  for(marker in colnames(exp.db)) {
    ...
    myScatterPlot2(legend=sprintf("cindex=%s",round(as.numeric(uhn.models[marker, effect.size]), digits=2)), Name=file.path("result/scatter", sprintf("%s_%s_aac.pdf", drug, marker)), 
                  x=predict(uhn.model), 
                  y=sensitivity, 
                  method="plain", 
                  minp=10,
...
for(drug in names(biomarkers)) {
  ...
  if (length(xx) > 0) {
    for(x in xx) {
      ...
      fnPlotHeatMap(sensitivity=uhn.auc, file.name=sprintf("%s_%s_image.pdf", gene, drug), cluster=FALSE, expression=expression, best.isoform=best.isoform)
      colnames(expression)[ncol(expression)] <- annot.gene$EnsemblGeneId
